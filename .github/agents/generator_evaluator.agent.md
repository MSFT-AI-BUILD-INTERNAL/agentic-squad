...
- **progress.json 초기값:**
  ...
  ```
- **산출물 파일명:** `{agent-name}-cycle{N}.md` (예: `generator-cycle1.md`)
- **Step 업데이트 로직:**
  1. `progress.json` 에서 현재 Cycle 의 해당 에이전트 status 를 `"completed"` 로 업데이트
  2. Evaluator 완료 시 `verdict` (`"pass"` / `"fail"`) 와 `scores` 를 기록. Fail 이면 Refiner 로 진행, Pass 이면 Scribe 로 진행
- **의사결정 드롭:** 없음
